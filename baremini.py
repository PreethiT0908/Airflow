...
# Each task block is indented with 4 spaces to sit inside `with DAG(...) as dag:`
_TASK_INDENT = "    "
...
def generate_dag_code(
    ...
    # ------------------------------------------------------------------
    # task_id_map: node_id -> sanitized task_id shown in the Airflow UI
    # The Airflow task_id uses the human-readable node NAME so the diagram
    # shows meaningful labels instead of opaque IDs like task1/task2.
    # node_id is kept internally for payload resolution and XCom keys.
    # ------------------------------------------------------------------
    task_id_map: Dict[str, str] = {}
    seen_task_ids: Set[str] = set()
    for node in all_nodes:
        base = sanitize_identifier(node["name"])
        if not base:
            base = sanitize_identifier(node["id"])
        candidate = base
        i = 2
    ...
    # node_id -> human name (for finalize_results summary)
    node_name_map = {node["id"]: node["name"] for node in all_nodes}
    # task_id -> node_id reverse map (for choose_branch xcom pull)
    task_id_to_node_id = {tid: nid for nid, tid in task_id_map.items()}

    # Build python variable names from the task_id (safe) and keep them unique.
    var_map: Dict[str, str] = {}  # node_id -> python variable name
    ...
    # Branch tasks: branch task_id is based on the name-based task_id
    branch_task_ids: Dict[str, str] = {
        node["id"]: f"branch_{task_id_map[node['id']]}"
        for node in all_nodes
        if node.get("branch_on_status")
    }
    ...
    # Dependencies in terms of node ids (internal graph edges)
    ...
    # Build upstream map to determine merge points (keyed by node_id)
    ...
    # ------------------------------------------------------------------
    # Build task definition blocks — each block is un-indented here;
    # _indent_block() will apply the correct indent before insertion.
    # ------------------------------------------------------------------
    task_def_blocks: List[str] = []

    for node in all_nodes:
        node_id = node["id"]
        task_id = task_id_map[node_id]   # name-based — shown in Airflow UI
        ...
        if node.get("branch_on_status"):
            ...
            # on_success/failure_task_ids for BranchPythonOperator must be
            # the Airflow task_ids (name-based), not node ids.
            success_tids = [task_id_map[ref] for ref in node.get("on_success_node_ids", [])]
            failure_tids = [task_id_map[ref] for ref in node.get("on_failure_node_ids", [])]
            b_block = (
                f"{branch_var} = BranchPythonOperator(\n"
                f"    task_id={branch_tid!r},\n"
                f"    python_callable=choose_branch,\n"
                f"    op_kwargs={{\n"
                f"        \"node_task_id\": {task_id!r},\n"
                f"        \"node_id\": {node_id!r},\n"
    ...
    # Helper to resolve python variable names
    special_task_vars = {
    ...
    # Dependency lines — also un-indented; will be indented before insertion
    dep_lines = [f"{to_var(parent)} >> {to_var(child)}" for parent, child in deduped_deps]

    # Apply 4-space indent to every block so they sit inside `with DAG(...) as dag:`
    indented_task_defs = "\n\n".join(
    ...
    template = f"""from datetime import datetime, timezone
from pathlib import Path
import hashlib
import json
import logging
...
TASK_ID_MAP = {repr(task_id_map)}
TASK_ID_TO_NODE_ID = {repr(task_id_to_node_id)}
BRANCH_SKIP_WHITELIST = {repr(sorted(branch_skip_whitelist))}
MERGE_NODE_IDS = {repr(sorted(merge_node_ids))}
...
def _mark_node_from_registry(ti, node_id, entry):
...
def _poll_status_until_terminal(node_id, node_name, payload, start_response_body, tracking_id):
    ...
    while time.time() <= deadline:
        try:
            response = requests.request(method=method, url=status_url, headers=headers, params=params, json=json_body, timeout=request_timeout, verify=verify_ssl)
        ...
        if response.status_code in (401, 403):
            raise AirflowException(f"{{node_id}} status endpoint authorization failed: HTTP {{response.status_code}}")
        if response.status_code == 404:
            raise AirflowException(f"{{node_id}} status endpoint returned 404")
        if response.status_code not in HTTP_SUCCESS_CODES:
            raise AirflowException(f"{{node_id}} status endpoint returned HTTP {{response.status_code}}: {{response.text}}")
        ...
        if normalized_status == "SUCCESS":
            return {{"status": "completed", "node_id": node_id, "tracking_id": tracking_id, "external_status": status_text, "start_response": start_response_body, "status_response": status_body}}
        if normalized_status == "FAILED":
            raise AirflowException(f"{{node_id}} external status failed. status_field={{response_status_key}}, value={{status_text}}")
        time.sleep(poke_interval)

    raise AirflowException(f"{{node_id}} status polling timed out after {{timeout_seconds}}s. Last response: {{last_status_response}}")
...
def execute_node(task_key, node_id, node_name, engine, execution_mode, **kwargs):
    ...
    if existing_entry and existing_entry.get("state") == "success" and not _should_force_rerun(conf, node_id):
        log.info("[IDEMPOTENCY] Reusing completed node %s for correlation_id=%s", node_id, _get_correlation_id(ctx))
        return _mark_node_from_registry(ti, node_id, existing_entry)

    if conf.get("resume"):
        log.info("[RESUME] resume=true, resume_from=%s, current_node=%s", conf.get("resume_from"), node_id)

    ti.xcom_push(key=f"{{node_id}}_task_state", value="started")
    ti.xcom_push(key=f"{{node_id}}_idempotency_key", value=node_key)
    _store_runtime_entry(node_key, {{"state": "started", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "started_at": _utc_now()}})

    request_timeout = int(payload.get("timeout", 300))
    verify_ssl = payload.get("verify_ssl", False)

    try:
        response = requests.request(method=payload.get("method", "POST"), url=payload["url"], json=payload.get("json"), params=payload.get("params"), headers=payload.get("headers", {{}}), timeout=request_timeout, verify=verify_ssl)
        response_body = _parse_response_body(response)
    except Exception as exc:
        ti.xcom_push(key=f"{{node_id}}_task_state", value="failed")
        ti.xcom_push(key=f"{{node_id}}_branch", value="failure")
        ti.xcom_push(key=f"{{node_id}}_error", value=str(exc))
        _store_runtime_entry(node_key, {{"state": "failed", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "error": str(exc), "finished_at": _utc_now()}})
        raise AirflowException(str(exc))

    if response.status_code in (401, 403):
        error_message = f"HTTP {{response.status_code}} for {{node_id}}. Check authentication/authorization."
        ti.xcom_push(key=f"{{node_id}}_task_state", value="failed")
        ti.xcom_push(key=f"{{node_id}}_branch", value="failure")
        ti.xcom_push(key=f"{{node_id}}_error", value=error_message)
        _store_runtime_entry(node_key, {{"state": "failed", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "http_status": response.status_code, "error": error_message, "finished_at": _utc_now()}})
    ...
    if response.status_code not in HTTP_SUCCESS_CODES:
        ...
        ti.xcom_push(key=f"{{node_id}}_branch", value="failure")
        ti.xcom_push(key=f"{{node_id}}_error", value=error_message)
        _store_runtime_entry(node_key, {{"state": "failed", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "http_status": response.status_code, "error": error_message, "response": response_body, "finished_at": _utc_now()}})
        raise AirflowException(error_message)
    ...
    if execution_mode == "fire_and_forget":
        ...
        ti.xcom_push(key=f"{{node_id}}_task_state", value="success")
        ti.xcom_push(key=f"{{node_id}}_branch", value="success")
        _store_runtime_entry(node_key, {{"state": "success", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "http_status": response.status_code, "submit_response": response_body, "result": result, "finished_at": _utc_now()}})
        return result
    ...
    if execution_mode == "async_no_wait":
        ...
        try:
            ...
            ti.xcom_push(key=f"{{node_id}}_branch", value="failure")
            ti.xcom_push(key=f"{{node_id}}_error", value=str(exc))
            _store_runtime_entry(node_key, {{"state": "failed", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "tracking_id": tracking_id, "submit_response": response_body, "error": str(exc), "finished_at": _utc_now()}})
            raise
        ti.xcom_push(key=f"{{node_id}}_response", value=result)
        ti.xcom_push(key=f"{{node_id}}_task_state", value="success")
        ti.xcom_push(key=f"{{node_id}}_branch", value="success")
        _store_runtime_entry(node_key, {{"state": "success", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "tracking_id": tracking_id, "http_status": response.status_code, "submit_response": response_body, "result": result, "finished_at": _utc_now()}})
        return result
    ...
    ti.xcom_push(key=f"{{node_id}}_task_state", value="success")
    ti.xcom_push(key=f"{{node_id}}_branch", value="success")
    _store_runtime_entry(node_key, {{"state": "success", "node_id": node_id, "node_name": node_name, "execution_mode": execution_mode, "dag_id": ctx["dag"].dag_id, "dag_run_id": ctx["dag_run"].run_id, "correlation_id": _get_correlation_id(ctx), "http_status": response.status_code, "result": result, "finished_at": _utc_now()}})
    return result
...
def choose_branch(node_task_id, node_id, success_task_ids, failure_task_ids, **kwargs):
    ctx = get_current_context()
    ti = ctx["ti"]
    # XCom key is keyed by node_id; task_ids= must be the Airflow task_id (name-based)
...
def finalize_results(node_ids, **kwargs):
    ...
    for node_id in node_ids:
        ...
        # Resolve the Airflow task_id (name-based) for this node_id
        airflow_task_id = TASK_ID_MAP.get(node_id, node_id)
        payload = resolve_payload(conf, node_id=node_id, task_key=node_id)

        # XCom task_ids= must use the Airflow task_id; key is still node_id-based
