...
# O = on-demand (no schedule, triggered externally via Airflow API)
# M = manual
...
class Node(BaseModel):
    ...
    #                       When you trigger a DAG run, conf["EDM_Location_Inbound_NAS_TO_PVC"]
    #                       must contain the url, method, json, etc. for that task.
...
def resolve_payload(conf, node_id, executor_build_id, task_key=None):
    \"\"\"
    Look up this node's payload from dag_run.conf.

    Lookup order:
      1. conf[executor_build_id]  -- primary key, e.g. "EDM_Location_Inbound_NAS_TO_PVC"
      2. conf[node_id]            -- fallback, e.g. "task1"
      3. conf[task_key]           -- legacy fallback
      4. scan for node_runId match

    The run-time conf must be structured as:
      {
        "correlation_id": "...",
        "EDM_Location_Inbound_NAS_TO_PVC": {
          "url": "http://...",
          "method": "POST",
          "json": { ... }
        }
    ...
    if executor_build_id:
        direct = conf.get(executor_build_id)
        if isinstance(direct, dict):
            return direct

    # Fallback 1: node_id (task1, task2, ...)
    direct = conf.get(node_id)
    if isinstance(direct, dict):
        return direct

    # Fallback 2: legacy task_key
    if task_key and task_key not in (node_id, executor_build_id):
        keyed = conf.get(task_key)
        if isinstance(keyed, dict):
            return keyed

    # Fallback 3: scan for node_runId match
...
def execute_node(
    ...
    **kwargs,
    ...
    payload = resolve_payload(conf, node_id=node_id, executor_build_id=executor_build_id, task_key=task_key)
