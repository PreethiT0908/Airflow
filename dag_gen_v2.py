"""
Dynamic Airflow DAG Service — airflow_job1 v1.2
Target: Airflow 3.0.6 (dev environment)

Changes in v1.2:
  - Runtime idempotency removed: no portalocker, no .lock file, no runtime_registry.json
    on the Airflow VM. Each task run is always executed fresh.
  - New payload schema: node `id` is now auto-generated as task1/task2... internally.
    Incoming payload may use either `id` (legacy) or a stable `executor_build_id` field.
    The `name` field no longer needs to be unique — `executor_build_id` is the stable key.
  - `completion` terminal node added at end of DAG. Mirrors finalize_results state:
    succeeds if finalize_results succeeded, fails otherwise. This makes the overall DAG
    state reflect the business outcome rather than the Kafka event result.
"""
...
app = FastAPI(title="Dynamic Airflow DAG Service", version="1.2.0-airflow306")
...
class Node(BaseModel):
    # --- NEW SCHEMA (v1.2) ---
    # `id` is now optional in the incoming payload.  When omitted it is
    # auto-assigned as "task1", "task2", … in payload-arrival order inside
    # BuildDagPayload.  If `id` is present it is still accepted (back-compat).
    #
    # `executor_build_id` is the stable, human-readable key that identifies
    # *what* this node runs (e.g. "EDM_Location_Inbound_NAS_TO_PVC").
    # It is embedded in generated DAG comments and XCom metadata for traceability.
    #
    # `name` is still the Airflow task_id.  It no longer needs to be globally
    # unique — uniqueness is now enforced on (executor_order_id, executor_sequence_id)
    # and on the auto-generated `id`.
    id: str = Field(default="", min_length=0)
    executor_build_id: Optional[str] = Field(default=None)
    ...
    @field_validator("executor_build_id", mode="before")
    @classmethod
    def normalize_executor_build_id(cls, value: Any) -> Optional[str]:
        if value is None:
            return None
...
class BuildDagPayload(BaseModel):
    ...
    def merge_and_assign_ids(self) -> "BuildDagPayload":
        """
        v1.2: Merge executionSteps + nodes into a single list, then
...
def generate_dag_code(
    ...
    # v1.2: task_id = node name, but deduplicate by appending _<order>_<seq> when names collide.
    ...
    executor_build_id_map = {node["id"]: node.get("executor_build_id") or node["name"] for node in all_nodes}
    ...
    for node in all_nodes:
        ...
        build_id = node.get("executor_build_id") or node["name"]
    ...
    terminal_node_ids = list(dict.fromkeys(final_layer_node_ids + branch_leaf_node_ids))
    ...
    template = f"""# Generated by dynamic-dag-service — airflow_job1 v1.2 — Airflow 3.0.6
...
# ---------------------------------------------------------------------------
# execute_node — v1.2: no runtime idempotency registry
# ---------------------------------------------------------------------------
...
def execute_node(
    ...
    **kwargs,
    ...
    if not payload or not payload.get("url"):
        ...
        raise AirflowException(
            f"Node {{node_id}} ({{node_name}} / build_id={{executor_build_id}}): "
            f"missing 'url' in conf. Expected conf key '{{node_id}}'."
        )
...
with DAG(
    ...
    tags=["dynamic", "generated", "v1.2", "airflow-3.0.6"],
