...
app = FastAPI(title="Dynamic Airflow DAG Service", version="10.0.0")
...
def make_task_id(node_id: str, node_name: str) -> str:
    """
    Build a human-readable Airflow task_id from the node name so that the
    Airflow graph UI shows meaningful labels on every node. The node_id is
    appended after a double-underscore to guarantee uniqueness and allow
    XCom key lookups to remain stable.
    """
...
def build_stage_dependencies(
    ...
    task_id_map: node_id -> airflow task_id (name-based)
    """
...
# ---------------------------------------------------------------------------
# Indentation helper (from baremini — used for clean block insertion)
# ---------------------------------------------------------------------------
...
def generate_dag_code(
    ...
    # Task IDs are derived from node NAME so Airflow graph nodes display
    # human-readable labels. The node_id suffix (after __) keeps them
    # unique and enables stable XCom key lookups.
    # ------------------------------------------------------------------
    task_id_map: Dict[str, str] = {}
    ...
    # task_id -> node_id reverse map (from baremini — useful inside generated code)
    ...
    # Branch task IDs — named after node task_id for clarity
    ...
    # Upstream map — detect merge nodes and set trigger rules
    ...
    # node_order_map: node_id -> executor_order_id (for deterministic resume skip)
    node_order_map: Dict[str, int] = {node["id"]: node["executor_order_id"] for node in all_nodes}

    # ------------------------------------------------------------------
    # Build task definition blocks with TaskGroup per executor_order_id
    # (from _1_ file) and proper trigger rules (merge nodes use
    # NONE_FAILED_MIN_ONE_SUCCESS from baremini, non-merge use ALL_SUCCESS)
    # ------------------------------------------------------------------

    # Build TaskGroup definitions (one per layer)
    ...
    for node in all_nodes:
        ...
        # Merge nodes get NONE_FAILED_MIN_ONE_SUCCESS (baremini) so that a
        # branch skip propagates correctly; all others use ALL_SUCCESS.
        ...
        doc_md_str = f"**{node_name_val}**  \\nengine: `{node_engine_val}`  \\nmode: `{node_mode_val}`"

        block = (
            f"with stage_{order_id}_group:\n"
    ...
    # Apply indentation using the helper from baremini
    indented_group_defs = "\n".join(
        _indent_block(line, _TASK_INDENT) for line in group_def_lines
    )
    indented_task_defs = "\n\n".join(
...
def execute_node(task_key, node_id, node_name, engine, execution_mode, executor_order_id, **kwargs):
    ...
    # Deterministic resume skip: when resume=true and resume_from is set,
    # fast-forward any node whose executor_order_id < resume_from node order.
    if conf.get("resume") and not _should_force_rerun(conf, node_id):
        ...
        if resume_from:
            ...
            log.info("[RESUME] resume=true, resume_from=%s, current_node=%s", resume_from, node_id)
